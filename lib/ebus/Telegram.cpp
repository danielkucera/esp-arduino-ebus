...
bool ebus::Telegram::isMaster(const uint8_t byte)
{
	...
	return (((hi == uint8_t(0x0)) || (hi == uint8_t(0x1)) || (hi == uint8_t(0x3)) || (hi == uint8_t(0x7))
		|| (hi == uint8_t(0xf)))
		&& ((lo == uint8_t(0x0)) || (lo == uint8_t(0x1)) || (lo == uint8_t(0x3)) || (lo == uint8_t(0x7))
			|| (lo == uint8_t(0xf))));
}
...
int ebus::Telegram::checkMasterSequence(Sequence &seq)
{
	...
	// number data byte is invalid
	if (uint8_t(seq[4]) > seq_max_bytes) return (SEQ_ERR_NN);

	// sequence is too short (incl. CRC)
	if (seq.size() < (size_t) (5 + uint8_t(seq[4]) + 1)) return (SEQ_ERR_SHORT);
...
int ebus::Telegram::checkSlaveSequence(Sequence &seq)
{
	...
	// number data byte is invalid
	if (uint8_t(seq[0]) > seq_max_bytes) return (SEQ_ERR_NN);

	// sequence is too short (incl. CRC)
	if (seq.size() < (size_t) (1 + uint8_t(seq[0]) + 1)) return (SEQ_ERR_SHORT);
