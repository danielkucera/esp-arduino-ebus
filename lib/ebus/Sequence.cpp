...
const std::vector<uint8_t> ebus::Sequence::range(const std::vector<uint8_t> &vec, const size_t index, const size_t len)
{
...
const std::vector<uint8_t> ebus::Sequence::to_vector(const std::string &str)
{
	...
	for (size_t i = 0; i + 1 < str.size(); i += 2)
		result.push_back(uint8_t(std::strtoul(str.substr(i, 2).c_str(), nullptr, 16)));
