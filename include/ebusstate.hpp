...
public:
    enum eState {
        ...
        eReceivedAddressAfterSYN,      // Received SYN ADDRESS
    ...
    void data(uint8_t symbol)
    {
        ...
        case eReceivedFirstSYN:
            _state = symbol == SYN ? eReceivedFirstSYN : eReceivedAddressAfterSYN;
            _master = symbol;
            break;
        case eReceivedAddressAfterSYN:
    ...
    {
        logmessage("unexpected SYN on bus while state is %s, setting state to %s\n", enumvalue(currentstate), enumvalue(newstate));
        return newstate;
    }
