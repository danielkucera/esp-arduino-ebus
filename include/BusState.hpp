...
class BusState {
 public:
  enum eState {
    ...
    eBusy  // Bus is busy; _master is master that won, _byte is first symbol
  ...
  BusState() : _state(eStartup), _previousState(eStartup) {}
  ...
  inline void data(uint8_t symbol) {
    switch (_state) {
      ...
      case eStartupFirstSyn:
        _previousState = _state;
        _state = symbol == SYN ? syn(eReceivedFirstSYN)
                               : eStartupSymbolAfterFirstSyn;
        break;
      ...
      case eReceivedSecondSYN:
        _previousState = _state;
        _state = symbol == SYN ? error(_state, eReceivedFirstSYN)
                               : eReceivedAddressAfterSecondSYN;
        _master = symbol;
        break;
  ...
  eState error(eState currentstate, eState newstate) {
    ...
    DEBUG_LOG(
        ...
        enumvalue(currentstate), enumvalue(newstate), _master, _symbol,
  ...
  uint32_t microsSinceLastSyn() const {
    return (uint32_t)(esp_timer_get_time()) - _SYNtime;
  }
